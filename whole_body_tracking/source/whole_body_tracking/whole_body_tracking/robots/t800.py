...
def _resolve_t800_urdf_path() -> str:
    ...
    for parent in this_file.parents:
        candidates.append(parent / "data" / "assets" / "assetslib" / "third_party" / "t800" / "urdf" / "serial_t800.urdf")

    for candidate in candidates:
        if candidate.is_file():
            return str(candidate)
    # Keep the legacy default for clearer downstream error messages if the file is still missing.
    return str(candidates[0])


T800_URDF_PATH = _resolve_t800_urdf_path()
