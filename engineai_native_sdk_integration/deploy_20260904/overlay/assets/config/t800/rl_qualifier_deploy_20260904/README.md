...
This directory packages the six-motion integration policy and 50 Hz reference trajectories for the independent
...
The state graph also includes EngineAI's official `pd_stand_x` and `pd_stand_y`
fall-recovery preparation poses. These PD states remain separate from the custom
recovery policy because their initial-pose contracts have not been shown to be
compatible.
