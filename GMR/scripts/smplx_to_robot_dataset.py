...
def process_file(
    ...
    num_pause = 0
    while check_memory(threshold_gb=memory_threshold_gb):
...
def main():
    ...
    parser.add_argument(
        ...
        default=30,
        type=float,
        help="Pause workers when available system memory falls below this value.",
    )
    parser.add_argument("--device", default="cuda:0", help="Torch device for FK post-processing.")
    ...
    with mp.Pool(args.num_cpus) as pool:
        pool.starmap(
            process_file,
            [arguments + (total_files, verbose, args.memory_threshold_gb, args.device) for arguments in args_list],
        )
